...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        no_rep = scheme_eval(first, env)
        check_procedure(no_rep)
        return scheme_apply(no_rep, rest.map(lambda r: scheme_eval(r, env)), env)
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        args_and_body = Pair(target.rest, expressions.rest)
        lamb_express = do_lambda_form(args_and_body,env)
        env.define(target_name, lamb_express)
        return target_name
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    def bindings_to_scm(bindings, position):
        if bindings==nil:
            return nil
        if position == 'first':
            return Pair(bindings.first.first, bindings_to_scm(bindings.rest, position))
        else:
            return Pair(scheme_eval(bindings.first.rest.first, env), bindings_to_scm(bindings.rest, position))
    formals = bindings_to_scm(bindings, 'first')
    check_formals(formals)
    values = bindings_to_scm(bindings, 'rest')
